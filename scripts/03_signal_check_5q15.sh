# scripts/03_signal_check_5q15.sh
#!/usr/bin/env bash
set -euo pipefail
...
req "${BFILE}.bim"; req "${BFILE}.bed"; req "${BFILE}.fam"
...
get_bp() {
  ...
  [[ -n "$bp" ]] || { echo "[ERR] SNP not in BIM: $rsid" >&2; exit 1; }
  echo "$bp"
}
...
plink_window() {
  ...
  # LD r2 to lead
  local ld_prefix="$OUTDIR/${tag}.ld_to_lead"
  local ld_file="${ld_prefix}.ld"
  if [[ -f "$ld_file" ]]; then
    echo "[SKIP] exists: $ld_file"
  else
    "$PLINK" \
      --bfile "$BFILE" \
      --chr 5 --from-bp "$from_bp" --to-bp "$to_bp" \
...
# Condition list for ERAP1 sig3
COND_S1S2="$OUTDIR/ERAP1_sig3_condition_list.txt"
printf "%s\n%s\n" "$ERAP1_SIG1" "$ERAP1_SIG2" > "$COND_S1S2"

# Run window scans
read -r ER2_ASSOC ER2_LD < <(plink_window "ERAP2" "ERAP2_sig1_baseline_pm${WINDOW_BP}" "$ERAP2_LEAD" "$er2_from" "$er2_to" "none")
read -r LN_ASSOC  LN_LD  < <(plink_window "LNPEP" "LNPEP_sig1_baseline_pm${WINDOW_BP}" "$LNPEP_LEAD" "$ln_from" "$ln_to" "none")

read -r S1_ASSOC  S1_LD  < <(plink_window "ERAP1" "ERAP1_sig1_baseline_pm${WINDOW_BP}" "$ERAP1_SIG1" "$s1_from" "$s1_to" "none")
read -r S2_ASSOC  S2_LD  < <(plink_window "ERAP1" "ERAP1_sig2_condS1_pm${WINDOW_BP}" "$ERAP1_SIG2" "$s2_from" "$s2_to" "snp" "$ERAP1_SIG1")
read -r S3_ASSOC  S3_LD  < <(plink_window "ERAP1" "ERAP1_sig3_condS1S2_pm${WINDOW_BP}" "$ERAP1_SIG3" "$s3_from" "$s3_to" "list" "$COND_S1S2")
...
# Single locus plots (titles as requested)
"$PYTHON" "$CODE_LOCUS" \
...
# 3-panel: ERAP2 sig1 + ERAP1 sig1 + LNPEP sig1
"$PYTHON" "$CODE_LOCUS" \
